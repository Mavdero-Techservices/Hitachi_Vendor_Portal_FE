    ...
    if (statutory?.length <= 0) {
      ...
      if (basicInfo?.length > 0 && basicInfo[0]?.Country_Region_Code && basicInfo[0].Country_Region_Code === 'IN') {
        ...
        Object.entries(statutory[0]).map(([key, value]) => {
          ...
          if(basicInfo[0]?.Country_Region_Code && basicInfo[0]?.Country_Region_Code === 'IN' && statutory[0].GST_Vendor_Type === "Registered" && key === "P_A_N_No" && value === "N/A"  )
          {
            statutoryArray.push("PAN NO is invalid");
          }
          if(basicInfo[0]?.Country_Region_Code && basicInfo[0]?.Country_Region_Code === 'IN' && statutory[0].GST_Vendor_Type === "UnRegistered" && key === "P_A_N_No" && value === "N/A"  )
          {
            statutoryArray.push("PAN NO is invalid");
          }
        });
  ...
  useEffect(() => {
    ...
    if (params.userId) {
      ...
      apiService.getAllCollection(params.userId).then((res) => {
        ...
        if (
          res.data.basicInfo[0]?.submitStatus === "Submitted" && finalstatus !== "Approved" && JSON.parse(window.sessionStorage.getItem("jwt")).result.role !== "Admin"
        ) {
      ...
      apiService.getAllCollection(newuser).then((res) => {
        if (
          res.data.basicInfo[0]?.submitStatus === "Submitted" && JSON.parse(window.sessionStorage.getItem("jwt")).result.role !== "Admin"
        ) {
          setStyle("notEditable");
        }
        Object.entries(res.data.contactDetail).map(([key, value]) => {
